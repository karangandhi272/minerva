...
app.post('/api/schedule', authenticateToken, async (req, res) => {
  try {
    ...
    try {
      ...
      // Try to get weekly schedule directly
      let scheduleData = [];
      try {
        console.log('Attempting to get weekly schedule...');
        const weeklySchedule = await userMinerva.getWeeklySchedule({
          season: season.toLowerCase(),
          year
        });
      ...
      } catch (weeklyScheduleError) {
        console.log('Weekly schedule failed, trying registered courses:', weeklyScheduleError);
        
        // Fallback to registered courses if weekly schedule fails
        try {
          ...
          console.log('Registered courses also failed:', registeredCoursesError);
          scheduleData = [];
        }
      }
