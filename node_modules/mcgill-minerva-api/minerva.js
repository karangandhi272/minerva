  ...
  return function(promised_obj) {
    ...
    .then(function(registeredCoursesResponse) {
      ...
      function buildAddDropForm(currentCourses, sel, isDrop) {
        ...
        if (currentCourses && currentCourses.length > 0) {
          currentCourses.forEach(function(course) {
            if (course.crn) {
              ...
              var isBeingDropped = isDrop && crnArray.includes(course.crn.toString());
              ...
              formParts.push("start_date_in=" + encodeURIComponent(course.start_date || ""));
              formParts.push("end_date_in=" + encodeURIComponent(course.end_date || ""));
              ...
              formParts.push("LEVL=" + encodeURIComponent(course.level || ""));
              formParts.push("CRED=" + encodeURIComponent(course.credits || course.credit || ""));
              formParts.push("GMOD=" + encodeURIComponent(course.gmod || ""));
        ...
        // For adding courses, add new courses with RW status
        if (!isDrop) {
        ...
        // Add empty slots for additional courses (typically 10 empty slots)
        for (var i = 0; i < 10; i++) {
          formParts.push("RSTS_IN=RW");
        ...
        // Footer with counters and submit button
        ...
        console.log('Generated form data (first 500 chars):', formString.substring(0, 500));
...
function promiseViewCourse(selection) {
  return function(promised_obj) {
    ...
    var formData = {
      'goto_date_in': '09/01/' + (selection.year || '2025'),
      'start_date_in': '06/30/' + (selection.year || '2025'),
      'pers_id': '',
      'user_type': ''
    };
...
function promiseWeeklySchedule(options) {
  return function(promised_obj) {
    ...
    var form = {
      start_date_in: '09/01/' + (options.year || '2025'),
      pers_id: '',
      user_type: ''
    };

    console.log('Weekly schedule request:', { url, form });
