...
function promiseAddOrDropCourse(drop, selection) {
  return function(promised_obj) {
    // Updated form encoding based on actual Minerva form structure
    function formUrlEncode(sel) {
      var sn = utils.fmtSeason(sel.season);
      var termCode = (sel.year || '2025') + sn;

      // Start with the header boilerplate
      var form = [
        "term_in=" + termCode,
        "&RSTS_IN=DUMMY",
        "&assoc_term_in=DUMMY", 
      ...
      // For each CRN we want to modify, we need to add the appropriate entry
      _.forEach(sel.crn, function(crn) {
        if (drop) {
          // For dropping courses, set RSTS_IN to DW (Drop Web)
          form += [
            "&RSTS_IN=DW",
          ...
          // For adding courses, set RSTS_IN to RW (Register Web)
          form += [
            "&RSTS_IN=RW",
            "&CRN_IN=" + crn,
            "&assoc_term_in=",
      ...
      // Add empty RW entries (these seem to be placeholders for additional courses)
      // Based on the form data, there are typically 10 empty slots
      for (var i = 0; i < 10; i++) {
        form += [
          "&RSTS_IN=RW",
          "&CRN_IN=",
      ...
      // Add the footer
      form += [
        "&regs_row=" + (drop ? "5" : "0"), // This seems to indicate which row is being modified
        "&wait_row=0",
        "&add_row=10",
