  ...
  return function(promised_obj) {
    ...
    .then(function(registeredCoursesResponse) {
      ...
      function buildAddDropForm(currentCourses, sel, isDrop) {
        ...
        // Add entries for each currently registered course (without RSTS_IN value)
        _.forEach(currentCourses, function(course) {
          if (course.crn) {
            form += [
              ...
              "&CRN_IN=" + course.crn,
              "&start_date_in=" + (course.start_date || ""),
              "&end_date_in=" + (course.end_date || ""),
        ...
        // Add entries for courses being added/dropped
        _.forEach(sel.crn, function(crn) {
          if (isDrop) {
            // For dropping, we need to find the course and mark it as DW
            var courseToDropIndex = _.findIndex(currentCourses, {crn: crn});
            if (courseToDropIndex !== -1) {
              // The form structure shows DW entries mixed with the regular entries
              // We'll add it as a separate entry
              form += [
                "&RSTS_IN=DW",
                "&assoc_term_in=" + termCode,
                "&CRN_IN=" + crn,
            ...
            // For adding courses, set RSTS_IN to RW
            form += [
              "&RSTS_IN=RW",
              "&CRN_IN=" + crn,
              "&assoc_term_in=",
        ...
        // Add empty RW slots (10 total as shown in the form data)
        var emptySlots = 10 - (isDrop ? 0 : sel.crn.length);
        ...
        // Add the footer - regs_row seems to be the number of currently registered courses
        form += [
