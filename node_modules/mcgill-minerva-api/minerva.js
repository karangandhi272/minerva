  ...
  return function(promised_obj) {
    ...
    .then(function(registeredCoursesResponse) {
      ...
      function buildAddDropForm(currentCourses, sel, isDrop) {
        ...
        if (currentCourses && currentCourses.length > 0) {
          currentCourses.forEach(function(course) {
            if (course.crn) {
              ...
              // For ADD operations, skip courses that are being added to avoid duplicates
              var isBeingAdded = !isDrop && crnArray.indexOf(course.crn.toString()) !== -1;
              
              console.log('Processing current course:', { 
                crn: course.crn, 
                isBeingDropped: isBeingDropped, 
                isBeingAdded: isBeingAdded,
                shouldSkip: isBeingAdded
              });
              
              // Skip this course if it's being added (to prevent duplicates)
              if (isBeingAdded) {
                console.log('Skipping course being added to prevent duplicate:', course.crn);
                return;
              }
        ...
        // For adding courses, put them in the first empty slots (only if not dropping)
        if (!isDrop) {
