      ...
      function buildAddDropForm(currentCourses, sel, isDrop) {
        ...
        if (currentCourses && currentCourses.length > 0) {
          currentCourses.forEach(function(course) {
            if (course.crn) {
              ...
              // Use proper date format based on season
              var startDate, endDate;
              if (selection.season.toLowerCase() === 'w') {
                startDate = "01/06/" + (selection.year || '2025');
                endDate = "04/15/" + (selection.year || '2025');
        ...
        if (!isDrop) {
          crnArray.forEach(function(crn) {
            ...
            var startDate, endDate;
            if (selection.season.toLowerCase() === 'w') {
              startDate = "01/05/" + (selection.year || '2025');
              endDate = "04/15/" + (selection.year || '2025');
            } else if (selection.season.toLowerCase() === 's') {
              startDate = "05/01/" + (selection.year || '2025');
              endDate = "08/15/" + (selection.year || '2025');
...
function promiseWeeklySchedule(options) {
  return function(promised_obj) {
    ...
    // Set proper date based on season and year - winter runs in the following calendar year
    var startDate;
    if (options.season.toLowerCase() === 'w') {
      // Winter semester: term 202501 runs in calendar year 2026 (January 2026)
      var winterYear = parseInt(options.year || '2025') + 1;
    ...
    console.log('Weekly schedule request:', { url: fullUrl, season: options.season, year: options.year, calculatedStartDate: startDate, termCode: termCode });
...
Minerva.prototype = {
  ...
  viewCourse: function(selection) {
    ...
    function(promised_obj) {
      ...
      try {
        ...
        console.log('ViewCourse: Parse error:', parseError);tails: ' + parseError.message));
        console.log('ViewCourse: Parse error:', parseError);
        deferred.reject(new Error('Failed to parse course details: ' + parseError.message));
      }
    },    // failure callback

    // failure callbackViewCourse: Promise chain error:', err);
    function(err) {
  ...
  getWeeklySchedule: function(options) {
    var deferred = q.defer();    console.log('GetWeeklySchedule called with options:', options);

    console.log('GetWeeklySchedule called with options:', options);    login(this)
seWeeklySchedule(options))
    login(this)
    .then(promiseWeeklySchedule(options))
    .then(    // success callback
bj) {
    // success callbackedule: Parsing schedule from response');
    function(promised_obj) {
      console.log('WeeklySchedule: Parsing schedule from response'); result = Parser.parseWeeklySchedule(promised_obj.body);
      try {
        var result = Parser.parseWeeklySchedule(promised_obj.body);
        console.log('WeeklySchedule: Parsed result:', result);
        deferred.resolve(result);chedule: Parse error:', parseError);
      } catch (parseError) {le: ' + parseError.message));
    ...
    // failure callbackWeeklySchedule: Promise chain error:', err);
    function(err) {
