...
function promiseAddOrDropCourse(drop, selection) {
  return function(promised_obj) {
    ...
    .then(function(registeredCoursesResponse) {
      ...
      // Fix the Request.post call - remove the headers parameter
...
function promiseWeeklySchedule(options) {
  return function(promised_obj) {
    ...
    if (options.season == 'W') {
      gotodate = '09/01/' + (options.year || '2025');
      startdate = '06/30/' + (options.year || '2025');
    } else {
      gotodate = '01/05/' + (options.year || '2025');
      startdate = '';
    }
