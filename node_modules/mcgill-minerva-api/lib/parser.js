  ...
  parseWeeklySchedule: function(html) {
    ...
    try {
      ...
      targetTable.find('tr').each(function(rowIndex) {
        ...
        // Process each cell in the row - look at ALL cells including those with rowspan
        row.find('td').each(function(cellIndex) {
          var cell = $(this);
          var courseLinkInCell = cell.find('a[href*="P_CrseSchdDetl"]');
          
          if (courseLinkInCell.length > 0) {
            // For cells with rowspan, we need to determine the actual column
            var actualColumnIndex = cellIndex;
            
            // Check if this cell has a rowspan and adjust column calculation
            var colspan = parseInt(cell.attr('colspan')) || 1;
            var rowspan = parseInt(cell.attr('rowspan')) || 1;
            
            // Look at previous cells in this row to determine the actual column
            var columnsUsed = 0;
            cell.prevAll('td').each(function() {
              var prevColspan = parseInt($(this).attr('colspan')) || 1;
              columnsUsed += prevColspan;
            });
            actualColumnIndex = columnsUsed;
