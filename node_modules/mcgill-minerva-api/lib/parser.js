...
module.exports = {
  ...
  parseWeeklySchedule: function(html) {
    ...
    try {
      ...
      // Map header indices to day names - improved mapping
      var dayMap = {};
      ...
      // Process each row after the header - look at ALL rows, not just immediate children
      var rowspanTracker = {}; // Track cells that span multiple rows
      ...
      targetTable.find('tr').each(function(rowIndex) {
        ...
        row.find('td').each(function(cellIndex) {
          ...
          while (rowspanTracker[currentColumnIndex] && rowspanTracker[currentColumnIndex] > rowIndex) {
            currentColumnIndex++;
          }
          ...
          // Mark future rows as occupied by this cell's rowspan
          if (rowspan > 1) {
            for (var r = rowIndex + 1; r < rowIndex + rowspan; r++) {
              for (var c = 0; c < colspan; c++) {
                rowspanTracker[currentColumnIndex + c] = r;
              }
            }
          }
          ...
          if (courseLinkInCell.length > 0) {
            var day = dayMap[currentColumnIndex];
            console.log('Found', courseLinkInCell.length, 'course links in row', rowIndex, 'column:', currentColumnIndex, 'day:', day, 'rowspan:', rowspan, 'colspan:', colspan);
          ...
          // Move to next column(s) based on colspan
